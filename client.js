...
class TileEdge {
    constructor(hasConnect, rotationClockwise) {
        ...
        this._hasConnect = hasConnect; //rename as "has connect" is used for pathconnect and "isConnected" is used for attachment to board
        this.canAccess = true;  //_
        this.isConnected = false;   //_
        this.connectionValid = false; //_
    ...
    _calculateStartPoint() {
        ...
        return new Point(pointX, pointY).add(this.position ?? {x:0,y:0});
    }
    ...
    getEnds() {
        let startPoint = this._calculateStartPoint();
    ...
    getVector() {
        return new Path(this.getEnds().start, this.getEnds().end);
    }
...
class Segment {
    ...
    closeEnd(end) {
        if(this._ends.includes(end)) {
            end.canAccess = false;

            let open = false;
            this._ends.forEach(end => {
                if (end.canAccess) open = true;
            })
            this._open = open;
            return true;
        } else {
...
class Pathway {
    ...
    closeEdge(edge) {
        if(this._openEnds.includes(edge)) this._openEnds.splice(this._openEnds.indexOf(edge), 1);
        this._segments.forEach(segment => {
            segment.closeEnd(edge);
        })
    }
...
class Tile{
    ...
    rotate(steps) {
        console.log("--",this._edges.length,steps)
        this._edges.forEach(edge => {
...
class Board{
    constructor(edges, pathways) {
        ...
        this._unfinishedPathways = this._pathways.slice();
    }
    ...
    addTile(tileToAdd, edges) {
        ...
        tileToAdd.setPosition(this._accessibleEdges[edges.connectEdges[0].b].position.add(tileEdges[edges.connectEdges[0].t].position.subtract(tileToAdd.getPosition())))

        edges.connectEdges.forEach(edge => {
            if(tileEdges[edge.t].getOrientation() + this._accessibleEdges[edge.b].getOrientation() == 7) fits = true;
        })
        edges.connectEdges.forEach(edge=> {
            if(!tileEdges[edge.t].position == this._accessibleEdges[edge.b].position) fits = false;
        })

        //collision checking? :o

        if(!fits) return false

        //adding the tile segments to the board pathways

        edges.connectEdges.forEach(edge => {

            let tSegment, bPathway;

            let boardEdge = this._accessibleEdges[edge.b];
            let tileEdge = tileToAdd.getEdges()[edge.t];

            if (boardEdge.hasConnect()) {
                this.getPathways().forEach(p => {
                    if(p.getOpenEnds().includes(boardEdge)) bPathway = p;
                })
            }
            if (tileEdge.hasConnect()) {
                tileToAdd.getSegments().forEach(s => {
                    if(s.getEnds().includes(tileEdge)) tSegment = s;
                })
            }

            if(boardEdge.hasConnect() && tileEdge.hasConnect()) {
                tSegment.closeEnd(tileEdge);
                tileEdge.closeConValid();
                bPathway.closeEdge(boardEdge);
                boardEdge.closeConValid();
            } else if (boardEdge.hasConnect() || tileEdge.hasConnect()) {
                tSegment.closeEnd(tileEdge);
                bPathway.closeEdge(boardEdge);
                tileEdge.closeConInvalid();
                boardEdge.closeConInvalid();
            } else {
                tileEdge.closeConValid();
                boardEdge.closeConValid();
            }
        ...
        //adding the tile edges.connectEdges to the board

        this._edges.push(...tileToAdd.getEdges())
        tileEdges = tileEdges.concat(tileEdges)
        let eLen = edges.connectEdges.length
        for(let c = 0; c < eLen; c++) {
            this._accessibleEdges[edges.connectEdges[c].b].canAccess = false
            tileToAdd.getEdges()[edges.connectEdges[c].t].canAccess = false

            this._accessibleEdges.splice(edges.connectEdges[c].b, 1, ...tileEdges.slice(edges.connectEdges[c].t+1,
                (edges.connectEdges[c].t < edges.connectEdges[(c+1)%eLen].t) ? edges.connectEdges[(c+1)%eLen].t : edges.connectEdges[(c+1)%eLen].t+tileEdges.length/2))
        }
        tileToAdd.setOriginalConnects(edges.connectEdges)
